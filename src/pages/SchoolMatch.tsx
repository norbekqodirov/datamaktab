...
const STORAGE_KEY = 'data_quiz_v1';
...
export default function SchoolMatch() {
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ screen, current, answers }));
  ...
  const allAnswered = answers.length === QUESTION_COUNT && answers.every(a => a !== undefined && a !== null && a >= 0);
  const result = useMemo(() => (allAnswered ? computeResult(answers) : null), [answers, allAnswered]);
  ...
  const goNext = () => {
    if (current < QUESTION_COUNT - 1) setCurrent(current + 1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const answerLines = answers
      .map((ai, qi) => `${qi + 1}) ${q.questions[qi].options[ai].label}`)
      .join(' | ');
    const topTitles = result.topDims.map(d => q.dims[d].title).join(', ');
    ...
    try {
      const res = await fetch('/api/crm/lead', {
        ...
        body: JSON.stringify({
          ...
          class: q.questions[0].options[answers[0]].label,
  ...
  const progress = screen === 'intro' ? 0
    : screen === 'quiz' ? ((current + (answers[current] !== undefined ? 1 : 0)) / QUESTION_COUNT) * 100
    : 100;

  const selectedIdx = answers[current];
  const reveal = screen === 'quiz' && selectedIdx !== undefined && selectedIdx >= 0
    ? q.questions[current].options[selectedIdx]
    : null;
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-5 md:px-8 pt-28 md:pt-36 pb-20">
            ...
            <div className="mb-8">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[11px] font-extrabold tracking-[0.2em] uppercase text-primary">
                  {screen === 'quiz' ? `${q.question_label} ${current + 1} / ${QUESTION_COUNT}` : 'DATA'}
                </span>
                <span className="text-[11px] font-bold text-on-surface-muted">{Math.round(progress)}%</span>
              </div>
              <div className="h-1.5 rounded-full bg-slate-200 overflow-hidden">
                <div
                  className="h-full rounded-full editorial-gradient transition-all duration-500"
          ...
          {screen === 'intro' && (
            <div key="intro" className="anim-slide-up glass-card rounded-[2rem] p-7 md:p-12 text-center">
              ...
              <p className="text-on-surface-muted max-w-xl mx-auto leading-relaxed mb-8">
                {q.intro_desc}
              </p>
              ...
              <button onClick={() => setScreen('quiz')} className="btn-primary w-full sm:w-auto px-12 py-4 text-sm uppercase tracking-widest inline-flex items-center justify-center gap-2">
          ...
          {/* ───── QUIZ ───── */}
          {screen === 'quiz' && (
            <div key={`q-${current}`} className="anim-slide-up">
              <div className="glass-card rounded-[2rem] p-6 md:p-10">
                ...
                {q.questions[current].sub && (
                  <p className="text-on-surface-muted text-sm mb-6">{q.questions[current].sub}</p>
                )}
                {!q.questions[current].sub && <div className="mb-6" />}

                <div className="space-y-3">
                  {q.questions[current].options.map((opt, i) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${
                          ...
                          <span className="material-symbols-outlined text-xl">{QUIZ_META[current].options[i].icon}</span>
                        </span>
                ...
                {/* reveal — tanlangan javobga DATA javobi */}
                {reveal && (
                ...
                <div className="flex items-center gap-3 mt-7">
                  ...
                  <button
                    onClick={goNext}
                    disabled={selectedIdx === undefined || selectedIdx < 0}
                  ...
                  >
                    {current === QUESTION_COUNT - 1 ? q.see_result : q.next} <ArrowRight size={15} />
                  </button>
                </div>
              </div>
            </div>
          ...
          {screen === 'result' && result && (
            <div key="result" className="anim-slide-up space-y-6">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  {result.topDims.map((dim, i) => (
                    <div key={dim} className="glass-card rounded-3xl p-6 flex flex-col border border-primary/5">
                      ...
                      <h4 className="font-headline font-extrabold text-primary text-lg leading-snug mb-2">{q.dims[dim].title}</h4>
                      <p className="text-on-surface-muted text-xs leading-relaxed mb-4">{q.dims[dim].desc}</p>
                      <ul className="space-y-2 mb-5 flex-1">
                        {q.dims[dim].facts.map((f, fi) => (
              ...
              {/* ustuvorliklar diagrammasi */}
              <div className="glass-card rounded-3xl p-6 md:p-8">
                <h3 className="font-headline font-extrabold text-lg text-primary mb-5">{q.result.chart_title}</h3>
                <div className="space-y-4">
                  {rankDims(result.scores, result.percents).map(dim => {
                    const isTop = result.topDims.includes(dim);
                    return (
                      <div key={dim}>
                        <div className="flex justify-between items-center mb-1.5">
                          <span className={`text-xs font-bold ${isTop ? 'text-primary' : 'text-on-surface-muted'}`}>{q.dims[dim].title}</span>
                          <span className={`text-xs font-extrabold ${isTop ? 'text-secondary' : 'text-slate-400'}`}>{result.percents[dim]}%</span>
                        </div>
                        <div className="h-2.5 rounded-full bg-slate-100 overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-700 ${isTop ? 'editorial-gradient' : 'bg-slate-300'}`}
                            style={{ width: `${result.percents[dim]}%` }}
                          />
                        </div>
                      </div>
                    );
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  onClick={downloadPdf}
                  disabled={pdfLoading}
                  className="flex-1 inline-flex items-center justify-center gap-2 bg-white border-2 border-primary text-primary hover:bg-primary hover:text-white px-6 py-4 rounded-full font-bold text-xs uppercase tracking-widest transition-colors disabled:opacity-50"
                >
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder={q.result.form_name_ph}
                        autoComplete="name"
          ...
          {screen === 'thanks' && result && (
            <div key="thanks" className="anim-slide-up glass-card rounded-[2rem] p-8 md:p-12 text-center">
              ...
              <button
                onClick={downloadPdf}
                disabled={pdfLoading}
                className="inline-flex items-center justify-center gap-2 bg-white border-2 border-primary text-primary hover:bg-primary hover:text-white px-8 py-3.5 rounded-full font-bold text-xs uppercase tracking-widest transition-colors disabled:opacity-50 mb-8"
              >
          ...
          <div style={{ position: 'fixed', left: '-10000px', top: 0 }} aria-hidden="true">
            <div ref={pdfRef} style={{ position: 'relative', width: 794, background: '#ffffff', fontFamily: "'Inter','Manrope',Arial,sans-serif", color: '#0f172a' }}>
              ...
              <div style={{ padding: '0 40px' }}>
                    ...
                    {result.topDims.map((dim, i) => (
                      <div key={dim} style={{ flex: 1, border: '1.5px solid #e2e8f0', borderTop: '4px solid #03caff', borderRadius: 14, padding: '14px 16px', background: '#fbfcfe' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                          ...
                          <h3 style={{ margin: 0, fontSize: 12.5, fontWeight: 800, color: '#062bad', lineHeight: 1.25, fontFamily: "'Manrope',Arial,sans-serif" }}>{q.dims[dim].title}</h3>
                        </div>
                        {q.dims[dim].facts.map((f, fi) => (
                      ...
                      return (
                        <div key={dim} style={{ marginBottom: 10 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                            <span style={{ fontSize: 10.5, fontWeight: 700, color: isTop ? '#062bad' : '#64748b' }}>{q.dims[dim].title}</span>
                ...
                {/* 5 — javoblar sarlavhasi (keyingi blok bilan birga ko'chadi) */}
                ...
                {answers.map((ai, qi) => (
                  <div key={qi} data-pdf-block="true" data-gap-after="2.5" style={{ display: 'flex', gap: 11, alignItems: 'flex-start', border: '1px solid #e2e8f0', borderRadius: 12, padding: '9px 14px', background: qi % 2 === 0 ? '#fbfcfe' : '#ffffff' }}>
                    <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#e8edfb', color: '#062bad', fontSize: 11, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>{qi + 1}</div>
                    <div>
                      <p style={{ margin: 0, fontSize: 9.5, color: '#64748b', lineHeight: 1.4 }}>{q.questions[qi].q}</p>
                      <p style={{ margin: '3px 0 0', fontSize: 11, fontWeight: 700, color: '#062bad', lineHeight: 1.4 }}>{q.questions[qi].options[ai].label}</p>
                    </div>
                  </div>
                ))}
